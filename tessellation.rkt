...
(define (rotate-pt p angle-deg)
  (define ptx (pt-x p))
  (define pty (pt-y p))
  (define angle (/ (* angle-deg pi) 180))
  (define new-x (- (* (cos angle) ptx) (* (sin angle) pty)))
  (define new-y (+ (* (sin angle) ptx) (* (cos angle) pty)))
  (pt new-x new-y))
...
(define (rotate90 c)
  ...
  (define (rotate90-curve c)
    ...
    (list c (curve: closed? (map-bez bezs pt-rotate))))
...
(define (rotate-curve-lambda angle n)
  ...
  (lambda (c)
    (defm (curve closed? bezs) c)
    (map (lambda (n) (curve: closed? (map-bez bezs (pt-rotate n)))) (range n))))
